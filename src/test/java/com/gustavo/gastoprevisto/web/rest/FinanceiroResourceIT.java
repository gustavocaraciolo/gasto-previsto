...
public class FinanceiroResourceIT {
    ...
    @Test
    @Transactional
    public void getAllFinanceirosByFinanceiroIsEqualToSomething() throws Exception {
        // Initialize the database
        financeiroRepository.saveAndFlush(financeiro);
        TiposFinanceiro financeiro = TiposFinanceiroResourceIT.createEntity(em);
        em.persist(financeiro);
        em.flush();
        financeiro.setFinanceiro(financeiro);
        financeiroRepository.saveAndFlush(financeiro);
        Long financeiroId = financeiro.getId();

        // Get all the financeiroList where financeiro equals to financeiroId
        defaultFinanceiroShouldBeFound("financeiroId.equals=" + financeiroId);

        // Get all the financeiroList where financeiro equals to financeiroId + 1
        defaultFinanceiroShouldNotBeFound("financeiroId.equals=" + (financeiroId + 1));
    }
